...
import befit.example.com.favouriteplaces.interfaces.LongExtractor;
...
public class MyFunDapter<T> extends BaseAdapter implements Filterable {
    ...
    private LoadPlacesInterface loadPlaces;
    ...
    public MyFunDapter(Context context, List<T> dataItems, int layoutResource,
                       BindDictionary<T> dictionary, LoadPlacesInterface loadPlaces) {
        this(context, dataItems, layoutResource, null, dictionary, loadPlaces);
    }
    ...
    public MyFunDapter(Context context, List<T> dataItems, int layoutResource,
                       LongExtractor<T> idExtractor, BindDictionary<T> dictionary, LoadPlacesInterface loadPlaces) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (null == v) {
            ...
            holder.rlOpis = (RelativeLayout) v.findViewById(R.id.rlOpis);
            holder.rlAdres = (RelativeLayout) v.findViewById(R.id.rlAdres);
            holder.tvOpis = (TextView) v.findViewById(R.id.tvOpis);
        ...
        holder.rlAdres.setVisibility(View.GONE);
        holder.ivItem.setImageResource(R.drawable.ic_arrow_drop_down_black_24dp);

        holder.rlOpis.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(holder.rlAdres.getVisibility() == View.VISIBLE) {
                    holder.rlAdres.setVisibility(View.GONE);
                    holder.ivItem.setImageResource(R.drawable.ic_arrow_drop_down_black_24dp);
                }
                else{
                    if(!holder.tvOpis.getText().toString().equals("Brak dodanych miejsc")) {
                        holder.rlAdres.setVisibility(View.VISIBLE);
                        holder.ivItem.setImageResource(R.drawable.ic_arrow_drop_up_black_24dp);
                    }
                }
            }
        });

        holder.rlOpis.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
                ...
                alertDialog.setMessage("Chcesz usunąć miejsce?");

                alertDialog.setPositiveButton("Yes",
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {

                                loadPlaces.loadPlaces(holder.tvAddress.getText().toString());
